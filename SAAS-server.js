  ...
  <main class="accueil">
    ...
    <section class="accueil-etapes">
      ...
      <div class="etapes-grille">
        ...
        <div class="etape"><span class="etape-num">3</span><h3>Tes clients scannent</h3><p>Ils voient ton catalogue à jour, sur leur téléphone.</p></div>
      </div>
    </section>
...
<body class="fond-braise">
  ...
  if (chemin === '/dashboard/update' && req.method === 'POST') {
    ...
    req.on('data', chunk => (corps += chunk));
    req.on('end', () => {
      const { index, nom, prix, photo, categorie, description, disponible, heureDebut, heureFin } = querystring.parse(corps);
      const db = lireDB();
      db.commercants[slug].produits[Number(index)] = { nom, prix: Number(prix), photo: photo || '', categorie: categorie || '', description: description || '', disponible: disponible === 'on', heureDebut: heureDebut || '', heureFin: heureFin || '' };
      ecrireDB(db);
  ...
  if (chemin === '/dashboard/add' && req.method === 'POST') {
    ...
    req.on('data', chunk => (corps += chunk));
    req.on('end', () => {
      const { nom, prix, photo, categorie, description, heureDebut, heureFin } = querystring.parse(corps);
      const db = lireDB();
      const commercant = db.commercants[slug];
      ...
      commercant.produits.push({ nom, prix: Number(prix), photo: photo || '', categorie: categorie || '', description: description || '', heureDebut: heureDebut || '', heureFin: heureFin || '', dateAjout: Date.now() });
      ecrireDB(db);
      res.writeHead(302, { Location: `/dashboard?lang=${url.searchParams.get('lang') || 'fr'}` });
      res.end();
    });
