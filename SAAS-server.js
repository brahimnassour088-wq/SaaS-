...
function pageDashboard(commercant, slug, hote, messageMotDePasse) {
  ...
  const lignesProduits = commercant.produits.map((p, i) => `
    <form method="POST" action="/dashboard/update" class="ligne-produit">
      ...
      <input name="categorie" value="${p.categorie || ''}" placeholder="Catégorie (optionnel, ex: Boissons)" class="champ-photo">
      <input name="photo" value="${p.photo || ''}" placeholder="Lien de la photo (optionnel)" class="champ-photo">
  ...
  <main class="dash-page">
    ...
    <section class="dash-section">
      ...
      <form method="POST" action="/dashboard/add" class="form-ajout">
        ...
        <input name="photo" placeholder="Lien de la photo (optionnel)">
    ...
    <section class="dash-section">
      ...
      <p>Affiché sur ton menu, et utilisé pour le bouton "Commander" par WhatsApp.</p>
      <form method="POST" action="/dashboard/telephone" class="stack">
        <label>Numéro
    ...
    <section class="dash-section">
      <h2>Numéro Mobile Money</h2>
      <p>Affiché sur ton menu, avec un bouton "Commander" sur chaque produit qui ouvre WhatsApp.</p>
      <form method="POST" action="/dashboard/paiement" class="stack">
        <label>Numéro
          <input name="telephonePaiement" value="${commercant.telephonePaiement || ''}" placeholder="Ex: 65 00 84 85">
        </label>
        <button type="submit" class="btn-primary">Enregistrer</button>
    ...
    <section class="dash-section">
      ...
      ${messageMotDePasse ? `<p class="form-error" style="color:${messageMotDePasse.startsWith('Mot de passe modifié') ? 'var(--amber-400)' : 'var(--ember-500)'};">${messageMotDePasse}</p>` : ''}
      <form method="POST" action="/dashboard/mot-de-passe" class="stack">
...
const NOMS_LANGUES = { fr: 'Français', en: 'English', ar: 'العربية' };

function pageMenuPublic(commercant, langue) {
  ...
  function ligneProduit(p) {
    const messageWa = encodeURIComponent(`Bonjour, je veux commander : ${p.nom} (${p.prix} F)`);
    return `
    <li class="menu-item">
...
<body class="fond-braise">
  <main class="menu-page">
    <div class="menu-card">
      <a class="lang-switch" href="?lang=${autreLangue}">${labelAutreLangue}</a>
...
const serveur = http.createServer((req, res) => {
  ...
  if (chemin.startsWith('/menu/') && req.method === 'GET') {
    ...
    return res.end(pageMenuPublic(commercant, langue));
  }
  ...
  if (chemin === '/dashboard/update' && req.method === 'POST') {
    ...
    req.on('data', chunk => (corps += chunk));
    req.on('end', () => {
      const { index, nom, prix, photo, categorie } = querystring.parse(corps);
      const db = lireDB();
      db.commercants[slug].produits[Number(index)] = { nom, prix: Number(prix), photo: photo || '', categorie: categorie || '' };
      ecrireDB(db);
  ...
  if (chemin === '/dashboard/mot-de-passe' && req.method === 'POST') {
    ...
    req.on('end', () => {
      ...
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
      return res.end(pageDashboard(db.commercants[slug], slug, req.headers.host, message));
    });
    return;
  }
