...
<body class="fond-braise">
  <main class="auth-page">
    <div class="auth-card">
      <span class="eyebrow">Espace propriétaire</span>
...
<html lang="fr">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>Espace propriétaire</title><link rel="stylesheet" href="/style.css"></head>
<body class="fond-braise">
...
<body class="fond-braise">
  <main class="accueil">
    <header class="accueil-nav">
      <span class="accueil-marque">KAVI<span class="point-ember">O</span></span>
    ...
    <section class="accueil-tarifs">
      ...
      <div class="tarifs-grille">
        ...
        <div class="tarif-carte tarif-carte-vedette">
          ...
          <a class="btn-primary tarif-btn" href="https://wa.me/23565008485?text=${encodeURIComponent("Bonjour, je veux passer à l'offre Pro (2500F/mois) sur KAVIO.")}" target="_blank">Passer en Pro</a>
        </div>
        <div class="tarif-carte">
          <h3>Business</h3>
...
const TRADUCTIONS = {
  fr: { menu: 'Menu', prix: 'Prix en Francs CFA', paiement: 'Paiement Mobile Money', commander: 'Commander', avis: 'Avis clients', laisserAvis: 'Laisser un avis', votreNote: 'Votre note', votreAvisOpt: 'Votre commentaire (optionnel)', envoyerAvis: 'Envoyer', merciAvis: 'Merci pour votre avis !', aucunAvis: 'Aucun avis pour le moment.' },
  en: { menu: 'Menu', prix: 'Prices in CFA Francs', paiement: 'Mobile Money payment', commander: 'Order', avis: 'Customer reviews', laisserAvis: 'Leave a review', votreNote: 'Your rating', votreAvisOpt: 'Your comment (optional)', envoyerAvis: 'Send', merciAvis: 'Thanks for your review!', aucunAvis: 'No reviews yet.' },
  ar: { menu: 'القائمة', prix: 'الأسعار بالفرنك الأفريقي', paiement: 'الدفع عبر موبايل موني', commander: 'اطلب', avis: 'آراء الزبائن', laisserAvis: 'أضف رأيك', votreNote: 'تقييمك', votreAvisOpt: 'تعليقك (اختياري)', envoyerAvis: 'إرسال', merciAvis: 'شكرا على رأيك!', aucunAvis: 'لا توجد آراء بعد.' },
};
...
const TRAD_DASH = {
  fr: {
    ...
    disponibleDe: 'Visible de', horaireA: 'à',
  },
  en: {
    ...
    disponibleDe: 'Visible from', horaireA: 'to',
  },
  ar: {
    ...
    disponibleDe: 'مرئي من', horaireA: 'إلى',
  },
};
...
const serveur = http.createServer((req, res) => {
  ...
  if (chemin === '/dashboard' && req.method === 'GET') {
    ...
    const langue = url.searchParams.get('lang') || 'fr';
    res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
    return res.end(pageDashboard(commercant, slug, req.headers.host, langue));
  }
...
<body class="fond-braise">
  ...
  if (chemin === '/dashboard/update' && req.method === 'POST') {
    ...
    req.on('data', chunk => (corps += chunk));
    req.on('end', () => {
      const { index, nom, prix, photo, categorie, disponible, heureDebut, heureFin } = querystring.parse(corps);
      const db = lireDB();
      db.commercants[slug].produits[Number(index)] = { nom, prix: Number(prix), photo: photo || '', categorie: categorie || '', disponible: disponible === 'on', heureDebut: heureDebut || '', heureFin: heureFin || '' };
      ecrireDB(db);
  ...
  if (chemin === '/dashboard/add' && req.method === 'POST') {
    ...
    req.on('data', chunk => (corps += chunk));
    req.on('end', () => {
      const { nom, prix, photo, categorie, heureDebut, heureFin } = querystring.parse(corps);
      const db = lireDB();
      db.commercants[slug].produits.push({ nom, prix: Number(prix), photo: photo || '', categorie: categorie || '', heureDebut: heureDebut || '', heureFin: heureFin || '' });
      ecrireDB(db);
      res.writeHead(302, { Location: `/dashboard?lang=${url.searchParams.get('lang') || 'fr'}` });
      res.end();
    });
