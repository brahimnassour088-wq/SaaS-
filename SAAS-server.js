...
function pageDashboard(commercant, slug, hote, messageMotDePasse) {
  ...
  const lignesProduits = commercant.produits.map((p, i) => `
    <form method="POST" action="/dashboard/update" class="ligne-produit ${p.disponible === false ? 'produit-epuise' : ''}">
...
<body class="fond-braise">
  <main class="dash-page">
    ...
    <section class="dash-section">
      <h2>QR code par table</h2>
      <p>Génère un QR code différent pour chaque table, le bar, ou "à emporter". Le serveur saura d'où vient la commande.</p>
      <form method="GET" action="/dashboard/qr-table" class="stack">
        <label>Nom (ex: Table 3, VIP, Bar, Emporter)
          <input name="nom" required placeholder="Table 1">
        </label>
        <button type="submit" class="btn-primary">Générer ce QR code</button>
      </form>
    </section>
