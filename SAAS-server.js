...
<body class="fond-braise">
  ...
  <script>
    // Upload photo depuis la galerie : compresse l'image côté téléphone
    // avant de la glisser dans le champ texte (en base64), pas besoin
    // d'hébergement externe.
    (function() {
      const TAILLE_MAX = 800; // largeur/hauteur max en pixels
      const QUALITE = 0.72;
      ...
      document.querySelectorAll('.input-photo-fichier').forEach(function(fichierInput) {
        fichierInput.addEventListener('change', function() {
          ...
          if (bouton) { bouton.textContent = '⏳'; bouton.disabled = true; }
          const lecteur = new FileReader();
          lecteur.onload = function(e) {
            const img = new Image();
            img.onload = function() {
              let largeur = img.width, hauteur = img.height;
              if (largeur > hauteur && largeur > TAILLE_MAX) {
                hauteur = Math.round(hauteur * (TAILLE_MAX / largeur));
                largeur = TAILLE_MAX;
              } else if (hauteur > TAILLE_MAX) {
                largeur = Math.round(largeur * (TAILLE_MAX / hauteur));
                hauteur = TAILLE_MAX;
              }
              ...
              champTexte.value = toile.toDataURL('image/jpeg', QUALITE);
              if (bouton) { bouton.textContent = '✅'; bouton.disabled = false; setTimeout(function() { bouton.textContent = '📷'; }, 1500); }
            };
...
const TRAD_DASH = {
  fr: {
    ...
    categorieOpt: 'Catégorie (optionnel)', photoOpt: 'Lien de la photo (optionnel)', prixLabel: 'Prix', prixPromoLabel: 'Prix promo (optionnel)',
    ...
    aProposTitre: 'À propos de ton commerce', aProposDesc: 'Ces infos apparaissent sur ta page catalogue, en bas.',
  ...
  en: {
    ...
    categorieOpt: 'Category (optional)', photoOpt: 'Photo link (optional)', prixLabel: 'Price', prixPromoLabel: 'Sale price (optional)',
    ...
    aProposTitre: 'About your business', aProposDesc: 'This info appears at the bottom of your catalog page.',
  ...
  ar: {
    ...
    categorieOpt: 'الفئة (اختياري)', photoOpt: 'رابط الصورة (اختياري)', prixLabel: 'السعر', prixPromoLabel: 'سعر العرض (اختياري)',
    ...
    aProposTitre: 'عن متجرك', aProposDesc: 'تظهر هذه المعلومات أسفل صفحة الكتالوج الخاصة بك.',
...
<body class="fond-braise">
  ...
  if (chemin === '/dashboard/update' && req.method === 'POST') {
    ...
    req.on('end', () => {
      ...
      db.commercants[slug].produits[Number(index)] = { nom, prix: Number(prix), prixPromo: promoValeur, photo: photo || '', categorie: categorie || '', description: description || '', disponible: stockValeur !== null && stockValeur <= 0 ? false : disponible === 'on', heureDebut: heureDebut || '', heureFin: heureFin || '', stock: stockValeur, dateAjout: ancien.dateAjout };
      ecrireDB(db);
  ...
  if (chemin === '/dashboard/apropos' && req.method === 'POST') {
    ...
    req.on('data', chunk => (corps += chunk));
    req.on('end', () => {
      const { adresse, horaires, instagram, facebook } = querystring.parse(corps);
      const db = lireDB();
