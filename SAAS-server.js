...
function pageAdmin(db) {
  const lignes = Object.entries(db.commercants).map(([slug, c]) => {
    const suspendu = c.suspendu === true;
    const messageRappel = encodeURIComponent(`Bonjour ${c.nom}, petit rappel : il reste peu de jours avant l'échéance de votre abonnement mensuel. Merci de faire le nécessaire pour continuer à profiter du service.`);
...
const serveur = http.createServer((req, res) => {
  ...
  // --- Suppression définitive d'un commerçant ---
