...
const TRADUCTIONS = {
  fr: { menu: 'Menu', prix: 'Prix en Francs CFA', paiement: 'Paiement Mobile Money' },
  en: { menu: 'Menu', prix: 'Prices in CFA Francs', paiement: 'Mobile Money payment' },
  ar: { menu: 'القائمة', prix: 'الأسعار بالفرنك الأفريقي', paiement: 'الدفع عبر موبايل موني' },
};
...
const serveur = http.createServer((req, res) => {
  ...
  if (chemin === '/dashboard/update' && req.method === 'POST') {
    ...
    req.on('data', chunk => (corps += chunk));
    req.on('end', () => {
      const { index, nom, prix, photo } = querystring.parse(corps);
      const db = lireDB();
      db.commercants[slug].produits[Number(index)] = { nom, prix: Number(prix), photo: photo || '' };
      ecrireDB(db);
  ...
  if (chemin === '/dashboard/add' && req.method === 'POST') {
    ...
    req.on('data', chunk => (corps += chunk));
    req.on('end', () => {
      const { nom, prix, photo } = querystring.parse(corps);
      const db = lireDB();
      db.commercants[slug].produits.push({ nom, prix: Number(prix), photo: photo || '' });
      ecrireDB(db);
      res.writeHead(302, { Location: '/dashboard' });
      res.end();
    });
